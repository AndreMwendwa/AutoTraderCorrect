...
def fitness_nb(dv, t_data, X_data):
    ...
    for i in range(t_data.shape[0]):
        t = t_data[i]
        # logistic cumulative adoptions
        pred = a * np.exp(-c * np.exp(-b * t))
        diff = X_data[i] - pred
        rss += diff * diff
    return rss
...
def main(output_filename, bass_input_name):
    ...
    for zone in bass_input['ZoneID'].unique():
        ...
        x = df_zone.loc[(df_zone['months_passed_01_2021'] > 0)|(df_zone['2021-2024_Total_EV_Sales'] > 0), 'months_passed_01_2021'].values.astype(np.float64)
        y = df_zone.loc[(df_zone['months_passed_01_2021'] > 0)|(df_zone['2021-2024_Total_EV_Sales'] > 0), 'cumsum'].values.astype(np.float64)
        ...
        print(f"Zone {zone}: a={a_opt:.6f}, b={b_opt:.6f}, c={c_opt:.2f}")

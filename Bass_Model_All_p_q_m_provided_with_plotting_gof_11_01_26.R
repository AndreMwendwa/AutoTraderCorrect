...
date <- "12_01_2026"
output_folder <- paste0("Results_output_", date)
vcov_folder   <- paste0("vcovs_", date)
if (!dir.exists(output_folder)) dir.create(output_folder)
if (!dir.exists(vcov_folder))   dir.create(vcov_folder)

# File names
txt_file_name <- paste0("results_txt_m_p_q_provided_90_perc_", date, ".txt")
csv_name      <- paste0("results_csv_m_p_q_provided_90_perc_", date, ".csv")
...
start_params <- read_csv("best_parameter_market_size_90_perc_correct_rev2.csv_20260112_010014.csv")
...
for (zone in zone_list) {
  input_data <- bass_input %>%
    filter(ZoneID == zone, (months_passed_01_2021 > 0) | (`2021-2024` > 0))
  
  # negative_xs = sum(input_data$months_passed_01_2021 < 0)
  # browser()
  input_data$months_passed_01_2021 = input_data$months_passed_01_2021 + abs(min(input_data$months_passed_01_2021))
  # browser()
  
  # print(input_data$`2021-2024`)
  # print(negative_xs)
